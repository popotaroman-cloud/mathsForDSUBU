...
**ตัวอย่างจริงจาก ISLP** (ทำไม Bootstrap ถึง "วิเศษ"): ISLP ทดลอง 2 แบบเปรียบเทียบกัน โดยกำหนดค่าจริง $\sigma_X^2=1, \sigma_Y^2=1.25, \sigma_{XY}=0.5$ ซึ่งให้ $\alpha_{\text{true}} = 0.6$

1. **วิธีในอุดมคติ** (ทำได้เฉพาะ simulation เพราะรู้ population จริง): จำลอง dataset ใหม่ 1,000 ชุดจาก population จริง (แต่ละชุด n=100) แล้วคำนวณ α̂ ทุกครั้ง → ได้ $\bar{\alpha} = 0.5996$ (ใกล้ 0.6 มาก!) และ **SE(α̂) ≈ 0.083**
2. **Bootstrap** (ทำได้กับข้อมูลจริงที่มีแค่ 1 ชุด): สุ่ม resample **จาก dataset เดียวเดิม** 1,000 ครั้ง (with replacement) แล้วคำนวณ α̂\* ทุกครั้ง → ได้ **Bootstrap SE(α̂) ≈ 0.087**

**ผลลัพธ์ 0.083 กับ 0.087 ใกล้กันมาก!** — นี่คือหัวใจของ Bootstrap: ในชีวิตจริงเราไม่มีทางสุ่ม dataset ใหม่จาก population ได้ (มีข้อมูลแค่ชุดเดียว) แต่ Bootstrap จำลองความแปรปรวนของ estimator ได้แม่นยำใกล้เคียงกับวิธีในอุดมคติ โดยใช้แค่ข้อมูลที่มีอยู่ชุดเดียวเท่านั้น
